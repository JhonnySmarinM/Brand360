...
const BrandAIModule = () => {
  ...
  const handlePlanChange = async (plan) => {
    ...
    if (!projectDesc || projectDesc.length < 5) return;
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-6">
              ...
              <div className="text-center">
                <h3 className="text-xl font-bold">Processing brand DNA...</h3>
                <p className="text-slate-400 text-sm">Our AI is designing your future.</p>
              </div>
            </div>
          ) : (
            <>
                ...
                <div className="grid md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                  <div className="space-y-6">
                    <h2 className="text-5xl font-black leading-tight tracking-tight">
                      Where ideas become <span className="text-yellow-500 underline decoration-4 underline-offset-8">brands.</span>
                    </h2>
                    <p className="text-slate-500 text-lg leading-relaxed">
                      Describe your project in a few words. Brand360 takes care of aesthetics, naming, and messaging.
                    </p>
                    <div className="flex gap-4 pt-4">
                      <div className="p-4 bg-white rounded-2xl shadow-sm border border-slate-100 flex-1">
                        <p className="text-2xl font-bold">100%</p>
                        <p className="text-[10px] uppercase font-bold text-slate-400 tracking-tighter">Originality</p>
                      </div>
                      <div className="p-4 bg-white rounded-2xl shadow-sm border border-slate-100 flex-1">
                        <p className="text-2xl font-bold">Instant</p>
                        <p className="text-[10px] uppercase font-bold text-slate-400 tracking-tighter">Speed</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="glass-card p-8 rounded-[40px] shadow-2xl shadow-slate-200/50 border border-white">
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">What are you going to create?</label>
                        <textarea 
                          value={projectDesc}
                          ...
                          placeholder="E.g.: A coffee shop that only opens at night for nocturnal writers..."
                        />
                      </div>
                      
                      <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Values (keywords)</label>
                        <div className="flex flex-wrap gap-2 min-h-[48px] p-2 bg-slate-50/50 rounded-2xl border-2 border-slate-100 focus-within:border-slate-200 transition-all">
                          ...
                          <input 
                            ...
                            placeholder={keywords.length < 5 ? "Type and press Enter..." : ""}
                      ...
                      >
                        Generate identity <ArrowRight className="group-hover:translate-x-2 transition-transform" />
                      </button>
                    </div>
                  </div>
                </div>
              ...
              {step === 2 && (
                <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                  <div className="flex justify-between items-center">
                    <h2 className="text-3xl font-black">Brand proposals</h2>
                  ...
                  <div className="flex gap-4">
                    <button onClick={() => setStep(1)} className="px-8 py-5 rounded-3xl font-bold text-slate-400 hover:text-slate-600 transition-colors">Back</button>
                    <button 
                      onClick={() => setStep(3)}
              ...
              {step === 3 && (
                <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-top-8 duration-700">
                  <div className="text-center space-y-2">
                    <h2 className="text-4xl font-black">Consolidated identity</h2>
